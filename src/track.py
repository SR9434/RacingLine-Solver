...
class Track:
    ...
    def __init__(self, inner_points: np.ndarray, outer_points: np.ndarray):
        ...
        print(f"[Track v12.1] ✓ 初始化完成. 长度={self.total_length:.2f}m, 高差={np.ptp(self.z_dense):.1f}m")
    ...
    def _preprocess_boundaries(self):
        ...
        def clean_and_resample(pts):
            ...
            # 0.2m 精度
            d_new = np.linspace(0, d_3d[-1], int(d_3d[-1]/0.2))
    ...
    def _generate_centerline(self):
        ...
        try:
            self.tck, u = splprep(mid_pts.T, u=None, s=len(mid_pts)*0.2, k=3, per=self.per_flag)
        except:
            self.tck, u = splprep(mid_pts.T, u=None, s=len(mid_pts)*1.0, k=3, per=self.per_flag)
    ...
    def _compute_ray_intersection_widths(self):
        """射线求交 (2D 投影面)"""
        if self.is_closed:
            s_vals = np.linspace(0, self.total_length, int(self.total_length/0.2), endpoint=False)
        else:
            s_vals = np.linspace(0, self.total_length, int(self.total_length/0.2), endpoint=True)
        ...
        mode = 'wrap' if self.is_closed else 'interp'
        self.wl_smooth = savgol_filter(wl_raw, 11, 2, mode=mode)
        self.wr_smooth = savgol_filter(wr_raw, 11, 2, mode=mode)
        self.wl_smooth = np.maximum(self.wl_smooth, 1.0)
        self.wr_smooth = np.maximum(self.wr_smooth, 1.0)
    ...
    def _build_lookup_tables(self):
        self.s_dense = np.linspace(0, self.total_length, int(self.total_length/0.1))
    ...
    def plot_3d_preview(self):
        """
        绘制 3D 赛道预览图 (交互式) - 已修复 NoneType 错误
        """
        try:
            ...
            ax.plot(self.x_dense[::step_c], self.y_dense[::step_c], self.z_dense[::step_c], 
                    'g--', lw=1, alpha=0.6, label='Centerline')
            ...
            for s in s_vals:
                ...
                # 每一根线是一个 segment: [(x1,y1,z1), (x2,y2,z2)]
            ...
            self.plot_boundary_comparison()

    def plot_boundary_comparison(self):
        try:
            fig, ax = plt.subplots(figsize=(10, 8))
            ax.scatter(self.inner_dense[::5,0], self.inner_dense[::5,1], s=5, c='gray', alpha=0.3, label='Wall')
            ax.scatter(self.outer_dense[::5,0], self.outer_dense[::5,1], s=5, c='gray', alpha=0.3)
            ax.plot(self.x_dense, self.y_dense, 'g--', lw=1, label='Centerline')
            ax.axis('equal'); ax.legend(); ax.set_title("Track v12.1: 2D Projection")
            plt.show()
