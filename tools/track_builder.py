...
class TrackBuilderV19:
    ...
    def cb_undo(self, e): 
        if self.temp_points: 
            self.temp_points = []
            self._update_view_2d() 
        elif self.layers[self.active_layer]['segments']: 
            self.layers[self.active_layer]['segments'].pop()
            self.layers[self.active_layer]['closed'] = False
            self._update_view_2d()
            self._update_mesh()
